...
func main() {
	...
	err := filepath.Walk(targetPath, func(path string, info os.FileInfo, err error) error {
		...
		if !info.IsDir() && filepath.Ext(path) == ".go" && !strings.HasSuffix(filepath.Base(path), "_test.go") {
			...
			generateTestCases(ctx, path, model)
		}
		return nil
	})
	if err != nil {
...
func generateTestCases(ctx context.Context, path string, model string) {
	cfg := &packages.Config{
	...
	for _, fn := range funcs {
		...
		fmt.Printf("Generating test case for package %s\n, function %s\n%s\n%s\n", packageName, fn.Name, fn.Code, model)
		testCasesList = append(testCasesList, testCase{Name: fn.Name, Code: fn.Code})
